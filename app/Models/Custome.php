...
class Custome extends Model
{
    ...
    private static function getImageUrl($request)
    {
        ...
        self::$imageName = rand(1000000, 50000000) . '.' . self::$extension;
    ...
    {
        self::$customer = Custome::where('email', $request->user_name)
        ...
        if (self::$customer) {
            if (password_verify($request->password, self::$customer->password)) {
                ...
                Session::put('customer_name', self::$customer->fname.' '.self::$customer->lname);
